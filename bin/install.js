...
const GSD_CODEX_MARKER = '# Blitz Agent Configuration \u2014 managed by blitz-core installer';
const GSD_CODEX_HOOKS_OWNERSHIP_PREFIX = '# Blitz codex_hooks ownership: ';

// Copilot instructions marker constants
const GSD_COPILOT_INSTRUCTIONS_MARKER = '<!-- Blitz Configuration \u2014 managed by blitz-core installer -->';
const GSD_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /Blitz Configuration -->';

const CODEX_AGENT_SANDBOX = {
  'blitz-executor': 'workspace-write',
  'blitz-planner': 'workspace-write',
  'blitz-phase-researcher': 'workspace-write',
  'blitz-project-researcher': 'workspace-write',
  'blitz-research-synthesizer': 'workspace-write',
  'blitz-verifier': 'workspace-write',
  'blitz-codebase-mapper': 'workspace-write',
  'blitz-roadmapper': 'workspace-write',
  'blitz-debugger': 'workspace-write',
  'blitz-plan-checker': 'read-only',
  'blitz-integration-checker': 'read-only',
};
...
Then re-run: npx blitz-core-cc@latest
`);
    process.exit(1);
  }
}
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx blitz-core-cc [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--cursor${reset}                  Install for Cursor only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}-u, --uninstall${reset}           Uninstall Blitz (remove all Blitz files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx blitz-core-cc\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx blitz-core-cc --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx blitz-core-cc --gemini --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx blitz-core-cc --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx blitz-core-cc --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx blitz-core-cc --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx blitz-core-cc --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx blitz-core-cc --antigravity --local\n\n    ${dim}# Install for Cursor globally${reset}\n    npx blitz-core-cc --cursor --global\n\n    ${dim}# Install for Cursor locally${reset}\n    npx blitz-core-cc --cursor --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx blitz-core-cc --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx blitz-core-cc --codex --global --config-dir ~/.codex-work\n\n    ${dim}# Install to current project only${reset}\n    npx blitz-core-cc --claude --local\n\n    ${dim}# Uninstall Blitz from Cursor globally${reset}\n    npx blitz-core-cc --cursor --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / GEMINI_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR / CURSOR_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all blitz: references → blitz-)
  c = c.replace(/blitz:/g, 'blitz-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all blitz: references → blitz-)
  c = c.replace(/blitz:/g, 'blitz-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize blitz: -> blitz-.
  // This preserves rendered "next step" commands like "/blitz-execute-phase 17".
  return content.replace(/blitz:/gi, 'blitz-');
}
...
## C. Tool Usage
Use these Cursor tools when executing Blitz workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run Blitz workflow ${skillName}.`;
  if (frontmatter) {
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/blitz:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$blitz-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/blitz-help\b/g, '$blitz-help');
  return converted;
}
...
Blitz workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
Blitz workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:

Direct mapping:
- \`Task(subagent_type="X", prompt="Y")\` → \`spawn_agent(agent_type="X", message="Y")\`
- \`Task(model="...")\` → omit (Codex uses per-role config, not inline model selection)
- \`fork_context: false\` by default — Blitz agents load their own context via \`<files_to_read>\` blocks
...
function generateCodexAgentToml(agentName, agentContent) {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `Blitz agent ${resolvedName}`
  );
...
/**
 * Generate the Blitz config block for Codex config.toml.
...
function stripCodexGsdAgentSections(content) {
  return content.replace(/^\[agents\.blitz-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
}

/**
 * Strip Blitz sections from Codex config.toml content.
...
function stripGsdFromCodexConfig(content) {
  ...
  if (markerIndex !== -1) {
    // Has Blitz marker — remove everything from marker to EOF
    let before = content.substring(0, markerIndex);
    before = stripCodexHooksFeatureAssignments(before, codexHooksOwnership);
    // Also strip Blitz-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have Blitz-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.blitz-*] sections (from header to next section or EOF)
...
function getManagedCodexHooksOwnership(content) {
  ...
  const match = afterMarker.match(/^\r?\n# Blitz codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function stripLeakedGsdCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) =>
      section.path.startsWith('agents.blitz-') ||
      (
        section.path === 'agents' &&
...
/**
 * Merge Blitz config block into an existing or new config.toml.
 * Three cases: new file, existing with Blitz marker, existing without marker.
 */
function mergeCodexConfig(configPath, gsdBlock) {
  ...
  // Case 2: Has Blitz marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any Blitz-managed sections that leaked above the marker from previous installs
  ...
  // Case 3: No marker — append Blitz block
...
/**
 * Merge Blitz instructions into copilot-instructions.md.
...
function mergeCopilotInstructions(filePath, gsdContent) {
  ...
  // Case 2: Has Blitz markers — replace between markers
...
/**
 * Strip Blitz section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was Blitz-only).
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('blitz-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex Blitz install path (absolute, so subagents with empty $HOME work — #820)
  const codexGsdPath = `${path.resolve(targetDir, 'blitz-core').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/blitz-core prefix so path resolves to codex Blitz install
    content = content.replace(/~\/\.claude\/blitz-core\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/blitz-core\//g, codexGsdPath);
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. Blitz agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /blitz:command with /blitz-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/blitz:/g, '/blitz-');
 ...
 * OpenCode expects: command/blitz-help.md (invoked as /blitz-help)
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old blitz-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/gsd/debug/start.md -> command/blitz-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> blitz-help.md
...
function listCodexSkillNames(skillsDir, prefix = 'blitz-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous Blitz Codex skills to avoid stale command skills.
...
function copyCommandsAsCursorSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous Blitz Cursor skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous Blitz Copilot skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Recursively install Blitz commands as Antigravity skills.
...
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous Blitz Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/blitz:/gi, 'blitz-');
...
/**
 * Clean up orphaned files from previous Blitz versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/blitz-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to blitz-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'blitz-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to blitz-statusline.js in v1.9.0
    'blitz-intel-index.js',  // Removed in v1.9.2
    'blitz-intel-session.js',  // Removed in v1.9.2
    'blitz-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old Blitz statusline.js path
  // Only match the specific old Blitz path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1blitz-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/blitz-statusline.js)`);
  }
...
/**
 * Uninstall Blitz from the specified directory for a specific runtime
 * Removes only Blitz-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling Blitz from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove Blitz commands/skills
  if (isOpencode) {
    // OpenCode: remove command/blitz-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('blitz-') && file.endsWith('.md')) {
          fs.unlinkSync(path.join(commandDir, file));
          removedCount++;
        }
      }
      console.log(`  ${green}✓${reset} Removed Blitz commands from command/`);
    }
  } else if (isCodex || isCursor) {
    // Codex/Cursor: remove skills/blitz-*/SKILL.md skill directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('blitz-')) {
          fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
          skillCount++;
        }
      }
    ...
    // Codex-only: remove Blitz agent .toml config files and config.toml sections
    if (isCodex) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('blitz-') && file.endsWith('.toml')) {
    ...
    // Codex: clean Blitz sections from config.toml
    const configPath = path.join(targetDir, 'config.toml');
    if (fs.existsSync(configPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was Blitz-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(configPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Blitz sections from config.toml`);
      }
    }
    }
  } else if (isCopilot) {
    // Copilot: remove skills/blitz-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Copilot: clean Blitz section from copilot-instructions.md
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was Blitz-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Blitz section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/blitz-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Cursor: remove skills/blitz-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
  ...
  // 2. Remove blitz-core directory
  const gsdDir = path.join(targetDir, 'blitz-core');
  if (fs.existsSync(gsdDir)) {
    fs.rmSync(gsdDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed blitz-core/`);
  }

  // 3. Remove Blitz agents (blitz-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} Blitz agents`);
    }
  }

  // 4. Remove Blitz hooks
  const hooksDir = path.join(targetDir, 'hooks');
  if (fs.existsSync(hooksDir)) {
    const gsdHooks = ['blitz-statusline.js', 'blitz-check-update.js', 'blitz-check-update.sh', 'blitz-context-monitor.js', 'blitz-prompt-guard.js'];
    let hookCount = 0;
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} Blitz hooks`);
    }
  }

  // 5. Remove Blitz package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed Blitz package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove Blitz hooks and statusline)
  const settingsPath = path.join(targetDir, 'settings.json');
  if (fs.existsSync(settingsPath)) {
    let settings = readSettings(settingsPath);
    let settingsModified = false;

    // Remove Blitz statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
        settings.statusLine.command.includes('blitz-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed Blitz statusline from settings`);
    }

    // Remove Blitz hooks from SessionStart
    if (settings.hooks && settings.hooks.SessionStart) {
      const before = settings.hooks.SessionStart.length;
      settings.hooks.SessionStart = settings.hooks.SessionStart.filter(entry => {
        if (entry.hooks && Array.isArray(entry.hooks)) {
          // Filter out Blitz hooks
          const hasGsdHook = entry.hooks.some(h =>
            h.command && (h.command.includes('blitz-check-update') || h.command.includes('blitz-statusline'))
          );
          return !hasGsdHook;
        }
        return true;
      ...
      if (settings.hooks.SessionStart.length < before) {
        settingsModified = true;
        console.log(`  ${green}✓${reset} Removed Blitz hooks from settings`);
      }
    ...
    // Remove Blitz hooks from PostToolUse and AfterTool (Gemini uses AfterTool)
    for (const eventName of ['PostToolUse', 'AfterTool']) {
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName].filter(entry => {
          if (entry.hooks && Array.isArray(entry.hooks)) {
            const hasGsdHook = entry.hooks.some(h =>
              h.command && h.command.includes('blitz-context-monitor')
            );
            return !hasGsdHook;
          }
          return true;
    ...
    // Remove Blitz hooks from PreToolUse (prompt injection guard)
    if (settings.hooks && settings.hooks.PreToolUse) {
      const before = settings.hooks.PreToolUse.length;
      settings.hooks.PreToolUse = settings.hooks.PreToolUse.filter(entry => {
        if (entry.hooks && Array.isArray(entry.hooks)) {
          const hasGsdHook = entry.hooks.some(h =>
            h.command && h.command.includes('blitz-prompt-guard')
          );
          return !hasGsdHook;
        }
        return true;
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove Blitz permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('blitz-core')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed Blitz permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No Blitz files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} Blitz has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading Blitz reference docs
 * This prevents permission prompts when Blitz accesses the blitz-core directory
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  // Build the Blitz path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const gsdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/blitz-core/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/blitz-core/*`;
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for Blitz docs`);
}
...
const PATCHES_DIR_NAME = 'blitz-local-patches';
const MANIFEST_NAME = 'blitz-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isAntigravity = runtime === 'antigravity';
  const isCursor = runtime === 'cursor';
  const gsdDir = path.join(configDir, 'blitz-core');
  ...
  for (const [rel, hash] of Object.entries(gsdHashes)) {
    manifest.files['blitz-core/' + rel] = hash;
  }
  ...
  if (!isCodex && !isCopilot) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('blitz-') && file.endsWith('.js')) {
...
/**
 * Detect user-modified Blitz files by comparing against install manifest.
 * Backs up modified files to blitz-local-patches/ for reapply after update.
 */
function saveLocalPatches(configDir) {
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified Blitz file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
...
function reportLocalPatches(configDir, runtime = 'claude') {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'copilot')
      ? '/blitz-reapply-patches'
      : runtime === 'codex'
        ? '$blitz-reapply-patches'
        : runtime === 'cursor'
          ? 'blitz-reapply-patches (mention the skill name)'
          : '/blitz:reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Path prefix for file references in markdown content (e.g. blitz-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>blitz-core/bin/...
  ...
  // Save any locally modified Blitz files before they get wiped
  ...
  if (isOpencode) {
    ...
    // Copy commands/gsd/*.md as command/blitz-*.md (flatten structure)
    const gsdSrc = path.join(src, 'commands', 'gsd');
    copyFlattenedCommands(gsdSrc, commandDir, 'gsd', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/blitz-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('blitz-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/blitz-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd');
    ...
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('blitz-')).length;
      if (count > 0) {
  ...
  // Copy blitz-core skill with path replacement
  const skillSrc = path.join(src, 'blitz-core');
  const skillDest = path.join(targetDir, 'blitz-core');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal);
  if (verifyInstalled(skillDest, 'blitz-core')) {
    console.log(`  ${green}✓${reset} Installed blitz-core`);
  } else {
    failures.push('blitz-core');
  }
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    // Remove old Blitz agents (blitz-*.md) before copying new ones
  ...
  const changelogDest = path.join(targetDir, 'blitz-core', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'blitz-core', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor) {
    // Write package.json to force CommonJS mode for Blitz scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current Blitz version into the hook version header
  ...
  if (isCodex) {
    ...
    try {
      ...
      const updateCheckScript = path.resolve(targetDir, 'blitz-core', 'hooks', 'blitz-update-check.js').replace(/\\/g, '/');
      const hookBlock =
        `${eol}# Blitz Hooks${eol}` +
        `[[hooks]]${eol}` +
      ...
      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('blitz-update-check')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'blitz-core', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'blitz-statusline.js')
    : 'node ' + dirName + '/hooks/blitz-statusline.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'blitz-check-update.js')
    : 'node ' + dirName + '/hooks/blitz-check-update.js';
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'blitz-context-monitor.js')
    : 'node ' + dirName + '/hooks/blitz-context-monitor.js';
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'blitz-prompt-guard.js')
    : 'node ' + dirName + '/hooks/blitz-prompt-guard.js';
  ...
  if (!isOpencode) {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('blitz-check-update'))
    );
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('blitz-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('blitz-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('blitz-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('blitz-prompt-guard'))
    );
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  Blitz includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with Blitz statusline
`);
...
/**
 * Install Blitz for all selected runtimes
 */
