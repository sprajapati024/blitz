...
/**
 * Blitz Tools — CLI utility for Blitz workflow operations
 *
 * Replaces repetitive inline bash patterns across ~50 Blitz command/workflow/agent files.
 * Centralizes: config parsing, model resolution, phase lookup, git commits, summary verification.
 *
 * Usage: node blitz-tools.cjs <command> [args] [--raw]
 *
 * Atomic Commands:
  ...
  switch (command) {
    ...
    case 'extract-messages': {
      ...
      if (!projectArg || projectArg.startsWith('--')) {
        error('Usage: blitz-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.');
      }
