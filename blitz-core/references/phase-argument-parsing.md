...
```bash
PHASE_INFO=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" find-phase "${PHASE}" --raw)
```
