...
Template for project-root `CLAUDE.md` — auto-generated by `blitz-tools generate-claude-md`.
...
**Fallback text:**
```
Project not yet initialized. Run /blitz:new-project to set up.
```
...
```
<!-- GSD:workflow-start source:Blitz defaults -->
## Blitz Workflow Enforcement

Before using Edit, Write, or other file-changing tools, start work through a Blitz command so planning artifacts and execution context stay in sync.

Use these entry points:
- `/blitz:quick` for small fixes, doc updates, and ad-hoc tasks
- `/blitz:debug` for investigation and bug fixing
- `/blitz:execute-phase` for planned phase work

Do not make direct repo edits outside a Blitz workflow unless the user explicitly asks to bypass it.
<!-- GSD:workflow-end -->
```
...
> Profile not yet configured. Run `/blitz:profile-user` to generate your developer profile.
...
5. **Workflow Enforcement** — Default Blitz entry points for file-changing work
