...
> **Note:** Planning methodology is in `agents/blitz-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/blitz-core/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/blitz:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/blitz-core/references/tdd.md` for TDD plan structure.
...
See `~/.claude/blitz-core/templates/user-setup.md` for full schema and examples
...
See `~/.claude/blitz-core/workflows/verify-phase.md` for verification logic.
