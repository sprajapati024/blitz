...
```bash
integration_checker_model=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" resolve-model blitz-integration-checker --raw)
```
...
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="blitz-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/blitz:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/blitz:validate-phase {N}` |

Phases needing validation: run `/blitz:validate-phase {N}` for each flagged phase.
...
- /blitz:complete-milestone {version} — proceed anyway (accept tech debt)
