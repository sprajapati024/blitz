...
```bash
AUDIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/blitz:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/blitz:verify-work {phase}`
```
</step>
