...
```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init milestone-op)
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/blitz:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/blitz:new-milestone` first."
...
```bash
DETAIL=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Blitz ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```
Skill(skill="blitz:plan-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="blitz:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="blitz:plan-phase", args="${PHASE_NUM} --gaps")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `blitz:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="blitz:audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /blitz:audit-milestone to review, then /blitz:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /blitz:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="blitz:complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /blitz:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="blitz:audit-milestone")
