...
1. **blitz-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **blitz-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/blitz:discuss-phase --text`)
- Per-project: `blitz-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/blitz:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init phase-op "${PHASE}")
...
Use /blitz:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /blitz:plan-phase {X} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" resolve-model blitz-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/blitz-advisor-researcher.md for your role and instructions.
...
**Also available:**
- `/blitz:plan-phase ${PHASE} --skip-research` — plan without research
- `/blitz:ui-phase ${PHASE}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="blitz:plan-phase", args="${PHASE} --auto")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /blitz:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /blitz:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /blitz:plan-phase ${PHASE} --gaps
  ```
