...
- **Claude Code:** Uses `Task(subagent_type="blitz-executor", ...)` — blocks until complete, returns result
...
These are the valid Blitz subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- blitz-executor — Executes plan tasks, commits, creates SUMMARY.md
- blitz-verifier — Verifies phase completion, checks quality gates
- blitz-planner — Creates detailed plans from phase scope
- blitz-phase-researcher — Researches technical approaches for a phase
- blitz-plan-checker — Reviews plan quality before execution
- blitz-debugger — Diagnoses and fixes issues
- blitz-codebase-mapper — Maps project structure and dependencies
- blitz-integration-checker — Checks cross-phase integration
- blitz-nyquist-auditor — Validates verification coverage
- blitz-ui-researcher — Researches UI/UX approaches
- blitz-ui-checker — Reviews UI implementation quality
- blitz-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init execute-phase "${PHASE_ARG}")
...
Check if the current runtime is Copilot by testing for the `@blitz-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/blitz-core/workflows/execute-plan.md` **inline**
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For blitz-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @~/.claude/blitz-core/workflows/execute-plan.md
       @~/.claude/blitz-core/templates/summary.md
       @~/.claude/blitz-core/references/checkpoints.md
       @~/.claude/blitz-core/references/tdd.md
       </execution_context>
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a Blitz or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/blitz:execute-phase {phase}                # Continue remaining waves
/blitz:execute-phase {phase} --wave {next}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
| `gaps_found` | Present gap summary, offer `/blitz:plan-phase {phase} --gaps` |
...
Commit the file:
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/blitz:progress` and `/blitz:audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/blitz:verify-work` on it.
...
`/blitz:plan-phase {X} --gaps`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/blitz:verify-work {X}` — manual testing first
```

Gap closure cycle: `/blitz:plan-phase {X} --gaps` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/blitz:execute-phase {X} --gaps-only` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/blitz:progress` and `/blitz:audit-uat`.
```

```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/blitz:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/blitz-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/blitz:transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/blitz:progress — see updated roadmap
/blitz:discuss-phase {next} — discuss next phase before planning
/blitz:plan-phase {next} — plan next phase
/blitz:execute-phase {next} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not Blitz. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/blitz:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
