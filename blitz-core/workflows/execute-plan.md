...
@~/.claude/blitz-core/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via blitz-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="blitz-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn blitz-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/blitz-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/blitz-core/bin/blitz-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/blitz-core/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/blitz-core/bin/blitz-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/blitz-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/blitz-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/blitz-core/templates/summary.md`.
...
Update STATE.md using blitz-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state record-metric \
...
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using blitz-tools:

```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/blitz:execute-phase {phase}` + `/blitz:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/blitz:plan-phase {Z+1}` + `/blitz:verify-work {Z}` + `/blitz:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/blitz:complete-milestone` + `/blitz:verify-work` + `/blitz:add-phase` |
