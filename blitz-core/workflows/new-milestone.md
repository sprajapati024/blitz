...
- Check for MILESTONE-CONTEXT.md (from /blitz:discuss-milestone)
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/blitz:new-milestone --reset-phase-numbers`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/blitz:plan-phase` behavior. To change the default, use `/blitz:settings`.
...
Spawn 4 parallel blitz-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/blitz-core/templates/research-project/{FILE}
</output>
", subagent_type="blitz-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/blitz:discuss-phase [N]` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/blitz:plan-phase [N]` — skip discussion, plan directly
```
...
- [ ] blitz-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/blitz:discuss-phase [N]`
