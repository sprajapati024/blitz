...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/blitz:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/blitz:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a Blitz project. Run `/blitz:new-project` to initialize one."
...
status: pending
priority: P2
source: "promoted from /blitz:note"
created: {YYYY-MM-DD}
theme: general
---
...
<edge_cases>
1. **"list" as note text**: `/blitz:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `~/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/blitz:new-project`
