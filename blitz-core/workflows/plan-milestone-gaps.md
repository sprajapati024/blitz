<purpose>
Create all phases necessary to close gaps identified by `/blitz:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/blitz:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/blitz:audit-milestone` first.
```
...
PHASES=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/blitz:execute-phase {N}` — if plans already exist
...
`/blitz:audit-milestone` — re-audit to verify gaps closed
`/blitz:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/blitz:plan-phase` next
</success_criteria>
