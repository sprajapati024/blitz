...
Seeds auto-surface during /blitz:new-milestone when trigger conditions match the
...
This seed should be presented during `/blitz:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /blitz:new-milestone
