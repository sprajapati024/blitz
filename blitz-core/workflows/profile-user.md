...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing blitz-tools.cjs subcommands and the blitz-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/blitz-core/references/ui-brand.md (display patterns)
- @$HOME/.claude/blitz-core/agents/blitz-user-profiler.md (profiler agent definition)
- @$HOME/.claude/blitz-core/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/blitz-core/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/blitz-core/USER-PROFILE.md" "$HOME/.claude/blitz-core/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Blitz > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/blitz-core/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /blitz:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/blitz-core/bin/blitz-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/blitz-core/bin/blitz-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn blitz-user-profiler agent using Task tool:**

Use the Task tool to spawn the `blitz-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/blitz-core/references/user-profiling.md`
...
Reference: @$HOME/.claude/blitz-core/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/blitz-core/bin/blitz-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/blitz-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/blitz-core/bin/blitz-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/blitz-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/blitz-core/bin/blitz-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/blitz-core/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/blitz:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/blitz-core/USER-PROFILE.md" and jump to step 10.
...
**For /blitz:dev-preferences (if selected):**

```bash
node $HOME/.claude/blitz-core/bin/blitz-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /blitz:dev-preferences at $HOME/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/blitz-core/bin/blitz-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/blitz-core/bin/blitz-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any blitz-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/blitz-core/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Blitz > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/blitz-core/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /blitz:dev-preferences   $HOME/.claude/commands/gsd/dev-preferences.md
...
- [ ] Artifacts generated sequentially via blitz-tools.cjs subcommands
