...
<required_reading>
@~/.claude/blitz-core/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init resume)
...
**If `planning_exists` is false:** This is a new project - route to /blitz:new-project
</step>
...
- This is the primary resumption source — structured data from `/blitz:pause-work`
...
📋 [N] pending todos — /blitz:check-todos to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/blitz:execute-phase {phase})
   OR
1. Discuss Phase 3 context (/blitz:discuss-phase 3) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/blitz:plan-phase 3) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show command for user to run after clearing:
  ...
  **Also available:**
  - `/blitz:discuss-phase [N]` — gather context first
  - `/blitz:research-phase [N]` — investigate unknowns
