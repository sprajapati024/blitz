...
Write to a temp file: `/tmp/blitz-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/blitz-review-prompt-{phase}.md)" 2>/dev/null > /tmp/blitz-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/blitz-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/blitz-review-claude-{phase}.md
```

**Codex:**
```bash
codex -p "$(cat /tmp/blitz-review-prompt-{phase}.md)" 2>/dev/null > /tmp/blitz-review-codex-{phase}.md
```
...
Commit:
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/blitz:plan-phase --reviews)
</success_criteria>
