<purpose>
Interactive configuration of Blitz workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.gsd/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config-ensure-section
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state load)
...
- `workflow.ui_safety_gate` — prompt to run /blitz:ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /blitz:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /blitz:ui-phase first when frontend indicators detected." },
...
These settings apply to future /blitz:plan-phase and /blitz:execute-phase runs.

Quick commands:
- /blitz:set-profile <profile> — switch model profile
- /blitz:plan-phase --research — force research
- /blitz:plan-phase --skip-research — skip research
- /blitz:plan-phase --skip-verify — skip plan check
```
</step>
