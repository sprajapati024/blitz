...
```bash
CONFIG=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /blitz:complete-milestone (if last phase in milestone)
- /blitz:progress (to see what's next)
...
- /blitz:complete-milestone — if all phases in milestone are done
- /blitz:progress — see overall project state
- /blitz:execute-phase {next} — continue to next phase
</offer_next>
