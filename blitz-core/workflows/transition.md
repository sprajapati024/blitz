...
There is no `/blitz:transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/blitz:transition`.

**Valid user commands for phase progression:**
- `/blitz:discuss-phase {N}` — discuss a phase before planning
- `/blitz:plan-phase {N}` — plan a phase
- `/blitz:execute-phase {N}` — execute a phase
- `/blitz:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/blitz:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to blitz-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `blitz-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `blitz-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/blitz:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/blitz:discuss-phase [X+1] --auto")
...
`/blitz:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/blitz:plan-phase [X+1]` — skip discussion, plan directly
- `/blitz:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/blitz:discuss-phase [X+1]` — revisit context
- `/blitz:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/blitz:complete-milestone {version}")
