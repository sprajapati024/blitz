<purpose>
Check for Blitz updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether Blitz is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/blitz:update` targets the runtime that invoked it.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/blitz-core/VERSION" ] || [ -f "./$dir/blitz-core/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/blitz-core/VERSION"
    LOCAL_MARKER_FILE="./$dir/blitz-core/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/blitz-core/VERSION" ] || [ -f "$HOME/$dir/blitz-core/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/blitz-core/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/blitz-core/workflows/update.md"
...
```bash
npm view blitz-core-cc version 2>/dev/null
```
...
To update manually: `npx blitz-core-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of Blitz folders:
- `commands/gsd/` will be wiped and replaced
- `blitz-core/` will be wiped and replaced
- `agents/blitz-*` files will be replaced
...
- Custom agents not prefixed with `blitz-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any Blitz files directly, they'll be automatically backed up to `blitz-local-patches/` and can be reapplied with `/blitz:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y blitz-core-cc@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y blitz-core-cc@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y blitz-core-cc@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/blitz-update-check.json"
  rm -f "$HOME/$dir/cache/blitz-update-check.json"
done
```

The SessionStart hook (`blitz-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  Blitz Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/glittercowboy/blitz-core/blob/main/CHANGELOG.md)
```
</step>
...
Check for blitz-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /blitz:reapply-patches to merge your modifications into the new version.
```
