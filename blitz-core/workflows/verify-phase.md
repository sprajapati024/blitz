...
<required_reading>
@~/.claude/blitz-core/references/verification-patterns.md
@~/.claude/blitz-core/templates/verification-report.md
</required_reading>
...
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" roadmap get-phase "${phase_number}"
...
Use blitz-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use blitz-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" verify artifacts "$plan")
...
Use blitz-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" verify key-links "$plan")
...
See ~/.claude/blitz-core/templates/verification-report.md for complete template.
</step>
