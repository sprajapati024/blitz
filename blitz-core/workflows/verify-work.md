<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /blitz:plan-phase --gaps.
...
<template>
@~/.claude/blitz-core/templates/UAT.md
</template>
...
```bash
INIT=$(node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" init verify-work "${PHASE_ARG}")
...
Provide a phase number to start testing (e.g., /blitz:verify-work 4)
```
...
Commit the UAT file:
```bash
node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- `/blitz:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @~/.claude/blitz-core/workflows/diagnose-issues.md
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Blitz ► PLANNING FIXES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning planner for gap closure...
```

Spawn blitz-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /blitz:execute-phase
...
Spawn blitz-planner with revision context:
...
3. Abandon (exit, user runs /blitz:plan-phase manually)
...
`/clear` then `/blitz:execute-phase {phase} --gaps-only`
...
- [ ] If issues: blitz-planner creates fix plans (gap_closure mode)
- [ ] If issues: blitz-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/blitz:execute-phase --gaps-only` when complete
</success_criteria>
