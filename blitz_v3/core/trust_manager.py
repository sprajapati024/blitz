...
class TrustManager:
    ...
    def get_tone(self) -> str:
        ...
        return self._preferences.get(
            "tone", "sassy"
        )  # Default to sassy for personality
    ...
    def record_project(self, project_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        project_record = {
            "id": project_data.get("id") or f"proj_{uuid.uuid4().hex[:8]}",
    ...
    def _format_auto_message(
        ...
        if message_type == "starting":
            ...
            if data.get("milestone"):
                return (
                    f"{data.get('component', 'Progress')} {data.get('status', 'done')}."
                )
            return None
