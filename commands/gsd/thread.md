...
```
No threads found. Create one with: /blitz:thread <description>
```
</mode_list>
...
5. Commit:
   ```bash
   node "$HOME/.claude/blitz-core/bin/blitz-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /blitz:thread {slug}
   ```
</mode_create>
...
- Lighter weight than /blitz:pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /blitz:add-phase or /blitz:add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other Blitz structures
</notes>
