...
def send_telegram_notification(message: str) -> bool:
    ...
    telegram_available = (
        os.environ.get("CLAUDE_TELEGRAM_AVAILABLE", "false").lower() == "true"
    )
...
class ProgressStreamer:
    ...
    def __init__(
        self, project_dir: Path, update_callback: Callable = None, tone: str = "sassy"
    ):
        ...
        self.tone = tone  # "sassy", "chill", "pro", or "minimal"

    def next_update_in(self) -> int:
        """Seconds until next update"""
        if not self.last_update:
            return 0
        return max(0, self.update_interval - (time.time() - self.last_update))
    ...
    def _format_error_tone(
        self, error: str, suggestions: list, recommendation: str
    ) -> str:
        """Format error message based on tone setting"""
        suggestion_text = "\n".join(
...
What feels right?"""
        elif self.tone == "chill":
            return f"""So {error} happened...
...
Recommendation: {recommendation}"""
    ...
    def report_error(self, error: str, suggestions: list, recommendation: str) -> str:
        """Report error with recovery options (tone-aware)"""
        return self._format_error_tone(error, suggestions, recommendation)
    ...
    def mark_resolved(self, error_index: int, resolution: str):
        ...
        try:
            ...
            if 0 <= error_index < len(errors):
                ...
                errors[error_index]["resolved_at"] = datetime.now(
                    timezone.utc
                ).isoformat()
                self.errors_file.write_text(json.dumps(errors, indent=2))
        except:
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        error_info = recovery.handle_error(
            "API rate limit exceeded (429)", "data fetching"
        )
